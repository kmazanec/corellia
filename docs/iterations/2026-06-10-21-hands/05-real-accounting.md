...
**ID:** F-35 · **Iteration:** 03-hands · **Status:** Shipped (build/03-hands)
...
Built as planned plus review repairs: ceiling now gates every debit site (step loop, terraced scan, recheck, repair, and _run entry — serial overshoot closed to ADR-017's one-in-flight exception, pinned by tests), tool-leaf step tokens debit the tokens dimension, fallback price raised to $25/1M (true output worst-case for opus-class). Tree accumulator: TreeState threaded by reference through _run recursion; ceiling-reached emitted exactly once per tree.
