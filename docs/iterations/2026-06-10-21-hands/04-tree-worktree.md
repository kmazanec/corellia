...
**ID:** F-34 · **Iteration:** 03-hands · **Status:** Shipped (build/03-hands)
...
Built per the scope note (module only) plus review repairs: injective tree ids (sha1 suffix), exclude-file idempotency pinned, rename-out-of-scope pinned, redundant diff query dropped. collectTree stages everything by design (per-leaf scope enforced elsewhere; relies on repo ignore rules — ADR-016 posture, why-comment in code). Engine wiring landed in F-37 as planned.
