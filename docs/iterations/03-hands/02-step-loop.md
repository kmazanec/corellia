...
**ID:** F-32 · **Iteration:** 03-hands · **Status:** Shipped (build/03-hands)
...
Built as planned plus review repairs: per-call budget gate (a step returning more calls than remaining budget routes only what budget allows), transcript tail carried to the next attempt as a low-severity finding on the verdict (priorAttempt evidence), remaining-count injected via an appended context transcript message (prefix-stable — earlier messages never mutate). Engine is the single debitor; refusals also debit (a call is a call).
