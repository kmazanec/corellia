...
- [ ] **Transport layer: bounded retry + backoff/jitter on 429/5xx/timeout, each a retry event, no usage debit** — Delivers a retry wrapper around the step fetch: 429/5xx/timeout retried up to a small cap (~3) with exponential backoff + jitter via an injectable sleep seam; each retry appends a retry event; on eventual success the step proceeds; on exhaustion it fails into the attempt ladder carrying the transport error. No usage debited for retried-away calls. Satisfies AC-2 (two 429/5xx then success: step succeeds, exactly N retries with backoff, each an event, no attempt consumed) and AC-3 (retries exhausted -> step fails into attempt ladder carrying transport error). Test target: tests/brains/llm.step.test.ts — describe('step transport retries'). Contract touchpoint: FactoryEvent (emits barrier-frozen transport-retry event member, ADR-018).

- [ ] **Protocol layer: one corrective re-prompt on malformed tool-call output, debited; second malformation fails the step** — Delivers: when the model returns an unparseable/unknown-tool/bad-shape tool call, step() issues exactly one corrective re-prompt carrying the parse error (generalizing the existing callJson two-fetch re-ask), debits usage, and on a second consecutive malformation fails the step. Emits a malformation event. Satisfies AC-4 (one malformed payload -> one corrective re-prompt carrying the parse error, debited; second consecutive malformation fails the step). Test target: tests/brains/llm.step.test.ts — describe('step malformation'). Contract touchpoint: FactoryEvent (emits barrier-frozen malformation/re-prompt event member, ADR-018).
...
- [ ] **Convergence wiring: npm run live:hands script (live, behind OPENROUTER_API_KEY) — iteration done-when, not a unit gate** — Delivers an examples/live-hands.ts script + a `live:hands` package.json script that runs a live sonnet-class implement leaf on a fixture repo through the full F-31..F-35 loop, printing the run report (real token+dollar totals from event usage). This is scaffolding for the iteration convergence check; it is gated behind OPENROUTER_API_KEY and is NOT part of the unit suite. Satisfies iteration convergence check (AC-7/AC-8/AC-12 observed live; PRD risk #1 first evidence). Test target: examples/live-hands.ts (manual run; no automated test target). Contract touchpoint: none (consumes the assembled engine + brain; no new contract).
...
- **FactoryEvent (src/contract/events.ts, ADR-018 + ADR-017)** — Action: consumes. Signature: Barrier adds (not by F-36): a transport-retry event member, a malformation/re-prompt event member, and Usage fields {promptTokens, completionTokens, costUsd?} on brain-call/step events. F-36 EMITS these via the EventStore.append it is given; it does not add the members to the union. If the barrier has not frozen these members, F-36 cannot satisfy AC-2/AC-4/AC-12 — flagged as the top risk.
