...
- [ ] **Freeze the Metered/Usage contract and propagate through all brains** — Delivers: Usage and Metered<T> types exist; Brain interface returns Metered<...>; LlmBrain reads provider `usage` (prompt_tokens/completion_tokens, cost when reported) instead of discarding it; ScriptedBrain accepts per-response synthetic usage. Compiles end-to-end with no chars/4 yet removed. Satisfies: AC-1 (partial: usage is now read from the response). Tests: tests/brains/llm.test.ts (assert callCompletions surfaces usage from a stubbed fetch response carrying a usage block; assert missing usage → costUsd undefined). tests/brains/scripted.test.ts (assert scripted synthetic usage is returned alongside value). Run: npx vitest run tests/brains/llm.test.ts tests/brains/scripted.test.ts. Contracts: src/contract/brain.ts (introduces Usage + Metered<T>; extends Brain method signatures).
